...
proc walk_simple*(folderpath: string, recursive: bool = true, relative: bool = false, check_folders: bool = false): seq[string] {.exportpy.} =
  ...
  if likely(recursive):
    ...
    for item in walkDir(folderpath, relative, checkDir=check_folders): result.add item.path
